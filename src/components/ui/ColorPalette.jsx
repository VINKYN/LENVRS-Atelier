...
export default function ColorPalette({ isMobile = false }) {
  ...
  return (
    <div className={`flex flex-col flex-1 min-h-0 ${isMobile ? 'gap-1.5' : 'gap-3.5'}`}>
      ...
      {isWaveZone && (
        <div className={`flex items-center gap-1 ${isMobile ? 'p-0.5' : 'p-1'} bg-[#f4f4f6] rounded-none flex-shrink-0`}>
          <button
            ...
            className={`flex-1 ${isMobile ? 'py-1 text-[10px]' : 'py-1.5 text-xs'} rounded-none font-semibold transition-all ${
              waveSubMode === 'fabric'
      ...
      {/* 1. RECENT COLORS (Minimalist Sharp Squares) */}
      <div className={`flex flex-col ${isMobile ? 'gap-1 pb-1' : 'gap-2 pb-3'} border-b border-[#f0f0f0] flex-shrink-0`}>
      ...
      {/* 3. PANTONE SHARP SQUARES GRID (7-8 columns on mobile for compact layout) */}
