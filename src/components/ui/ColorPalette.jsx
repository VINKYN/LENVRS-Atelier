  ...
  return (
    <div className={`flex flex-col flex-1 min-h-0 ${isMobile ? 'gap-1' : 'gap-3.5'}`}>
      ...
      {isMobile ? (
        ...
        <div className="flex items-center justify-between py-0.5 border-b border-[#f0f0f0] flex-shrink-0">
          <div className="flex items-center gap-1 text-[10px] font-semibold text-[#707072]">
            <History className="w-2.5 h-2.5 text-[#8e8e93]" />
            <span>
              {isWaveZone && waveSubMode === 'topstitch' ? 'Récents fil' : 'Récents'}
            </span>
          </div>
            ...
            {recentColors.map((hex, idx) => {
              ...
              return (
                <button
                  ...
                  className={`w-4 h-4 rounded-none transition-all active:scale-95 ${
                    isSelected
        ...
        // Desktop Full Width Row (Exactly as before)
        <div className="flex flex-col gap-2 pb-3 border-b border-[#f0f0f0] flex-shrink-0">
          <div className="flex items-center gap-1.5 text-xs font-semibold text-[#707072]">
